...
def viewer(env, mode="initialize", filename="video", frame_size=(640, 480), camera_id=0, render=None):
    if render == "onscreen":
        ...
        if mode == "render":
            if hasattr(env, "render_cameras"):
                cams = env.render_cameras()
                curr_frame = np.concatenate(list(cams.values()), axis=1)
            else:
...
def gather_training_data(env, data, filename="demo_playback.mp4", render=None):
    ...
    for i_frame in range(data["ctrl"].shape[0] - 1):
        ...
        # Capture per-camera images for this timestep
        if hasattr(env, "render_cameras"):
            cams = env.render_cameras()
            for cam_name, frame in cams.items():
                camera_obs.setdefault(cam_name, []).append(frame)
    ...
    # Stack per-camera frame lists into arrays of shape (T, H, W, C)
    camera_obs = {name: np.stack(frames) for name, frames in camera_obs.items()}

    # note that <init_qpos, init_qvel> are one step away from <path_obs[0], path_act[0]>
    return path_obs, path_act, init_qpos, init_qvel, camera_obs
...
def main(env, demo_dir, skip, graph, save_logs, view, render):
    ...
    for ind, file in enumerate(glob.glob(demo_dir + "*.mjl")):
        ...
        # playback logs and gather data
            ...
            try:
                obs, act, init_qpos, init_qvel, camera_obs = gather_training_data(
                ...
                traceback.print_exc()
                continue
            observations = {"state": obs, **camera_obs}
            path = {"observations": observations, "actions": act, "init_qpos": init_qpos, "init_qvel": init_qvel}
            paths.append(path)
            pkl_path = file[:-4] + "_path.pkl"
