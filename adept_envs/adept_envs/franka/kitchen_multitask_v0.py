...
class KitchenTaskRelaxV1(KitchenV0):
    ...
    def _render_offscreen(self, camera_id, width=320, height=240):
        return super(KitchenTaskRelaxV1, self).render(mode="rgb_array", width=width, height=height, camera_id=camera_id)

    def render(self, mode="human"):
        if mode == "rgb_array":
            # camera_id 2 = scene_cam (front overview of robot + kitchen)
            return self._render_offscreen(camera_id=2)
        else:
            super(KitchenTaskRelaxV1, self).render()

    def render_cameras(self, width=320, height=240):
        """Return a dict of rendered views from all useful cameras."""
        return {
            "scene": self._render_offscreen(camera_id=2, width=width, height=height),
            "wrist": self._render_offscreen(camera_id=3, width=width, height=height),
        }
