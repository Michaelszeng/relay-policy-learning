...
if __name__ == "__main__":
    ...
    np.random.seed(43)
    # The live onscreen viewer needs a GLFW render backend + a display. Offscreen
    # backends (egl/osmesa, used on headless machines) can render the policy's
    # cameras but cannot open a window, so force headless to avoid a crash inside
    # mj_render(). Camera rendering for the policy is unaffected either way.
    _gl_backend = os.environ.get("MUJOCO_GL", "").lower()
    if not args.headless and _gl_backend in ("egl", "osmesa"):
        print(
            f"[main] MUJOCO_GL={_gl_backend!r} is an offscreen-only backend that "
            "can't open a live viewer window; forcing --headless."
        )
        args.headless = True
    if args.n_envs == 1:
